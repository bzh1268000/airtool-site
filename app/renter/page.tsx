...
export default function RenterPage() {
  ...
  // Tool purchases
  const [purchases, setPurchases] = useState<{ id: number; tool_name: string; sale_price: number; paid_at: string }[]>([]);
  ...
  useEffect(() => {
    const fetchRenterData = async (user: any) => {
      ...
      // Fetch tool purchases by this buyer
      const { data: purchaseData } = await supabase
        .from("tool_sales")
        .select("id, tool_name, sale_price, paid_at")
  ...
  const totalSpent = useMemo(
    () =>
      ...
      purchases.reduce((sum, p) => sum + Number(p.sale_price || 0), 0),
    [bookings, purchases]
  );
      ...
      <div className="grid gap-6">
            ...
            {sortedBookings.slice(0, 8).map((b) => (
              ...
              >
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  ...
                  {/* Review button — completed, resolved dispute, OR confirmed 7+ days unpaid (stuck zero-price) */}
                  {(() => {
                    const refDate      = b.confirmed_at || b.end_date;
                    const daysSinceRef = refDate
                      ? (Date.now() - new Date(refDate).getTime()) / (1000 * 60 * 60 * 24)
                      : 0;
                    const stuckConfirmed = b.status === "confirmed" && !b.paid_at && daysSinceRef >= 7;
                    return b.status === "completed" || disputesMap[b.id]?.status === "resolved" || stuckConfirmed;
                  })() && (
        ...
        <div className="mt-8 rounded-[28px] bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">🏷️ Tools You Own</h2>
              <p className="mt-0.5 text-sm text-black/50">Tools you have bought outright via AirTool.</p>
            </div>
            {purchases.length > 0 && (
              <div className="rounded-2xl border border-orange-100 bg-orange-50 px-4 py-2 text-center">
                <div className="text-lg font-bold text-orange-600">NZ${purchases.reduce((sum, p) => sum + Number(p.sale_price), 0).toFixed(2)}</div>
                ...
                <div key={p.id} className="rounded-2xl bg-orange-50 border border-orange-100 p-4">
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div className="text-right">
                      <div className="text-xl font-bold text-orange-600">NZ${Number(p.sale_price).toFixed(2)}</div>
                      <div className="text-xs text-green-600 font-semibold mt-1">✅ Paid</div>
                    </div>
                  </div>
                </div>
              ))}
